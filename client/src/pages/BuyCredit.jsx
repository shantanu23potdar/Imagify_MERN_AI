...
const BuyCredit = () => {
    ...
    const options = {
      ...
      handler: async(response)=>{
        try {
          ...
          const {data} = await axios.post(backendUrl + '/api/user/verify-razor', response, {headers: {Authorization:token}})
  ...
  const paymentRazorpay = async(planId) => {
    try {
      ...
      const {data} = await axios.post(backendUrl + '/api/user/pay-razor' ,{planId}, {headers : {Authorization: token}})
  ...
  return (
    <motion.div className='min-h-[80hv] text-center pt-14 mb-10'
